...
class RolloutManager:
    ...
    def _save_debug_rollout_data(self, data, rollout_id, evaluation: bool):
        ...
        if (path_template := self.args.save_debug_rollout_data) is not None:
            # Support both {rollout_id} and {wandb_run_id} in the template.
            formatted = path_template.format(
                rollout_id=("eval_" if evaluation else "") + str(rollout_id),
                wandb_run_id=self.wandb_run_id,
            )
            # If the formatted value points to a directory (or ends with a separator),
            # append a default filename to avoid saving to a directory path.
            if formatted.endswith("/") or Path(formatted).suffix == "":
                filename = f"rollout_{'eval_' if evaluation else ''}{rollout_id}.pt"
                path = Path(formatted) / filename
            else:
                path = Path(formatted)
        ...
        rollout_engine = RolloutRayActor.options(
            ...
            runtime_env={
                ...
                | {
                    ...
                    "SGLANG_MEMORY_SAVER_CUDA_GRAPH": "false",
                }
            },
        ).remote(args, rank=i)
